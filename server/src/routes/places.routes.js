...
import { parseTags, serializePlace, serializePlaceWithStats } from "../serialize.js";
...
router.post("/", requireAuth, async (req, res) => {
  ...
  for (const field of ["name", "description", "address", "city", "price", "openHours"]) {
...
router.post("/:id/rate", requireAuth, async (req, res) => {
  ...
  const rating = await prisma.rating.upsert({
    where: {
      userId_placeId: { userId: req.userId, placeId: req.params.id },
    },
    create: { userId: req.userId, placeId: req.params.id, value },
    update: { value },
  });
...
router.post("/:id/reservations", requireAuth, async (req, res) => {
  ...
  if (date.getTime() <= Date.now()) {
    return res.status(400).json({ error: "Tanggal reservasi harus di masa depan." });
  }
  if (!Number.isInteger(partySize) || partySize < 1 || partySize > 50) {
    return res.status(400).json({ error: "Jumlah orang harus antara 1 sampai 50." });
  }
