...
router.get("/me", requireAuth, async (req, res) => {
  const user = await prisma.user.findUnique({
    where: { id: req.userId },
    include: {
      _count: { select: { posts: true, ratings: true, comments: true, savedPosts: true } },
      achievements: {
        include: { badge: true },
      ...
      ratings: {
        include: { place: { select: { id: true, name: true, city: true, imageUrl: true } } },
        orderBy: { createdAt: "desc" },
      },
      comments: {
        include: { place: { select: { id: true, name: true, city: true, imageUrl: true } } },
  ...
  res.json({
    ...
    savedPosts: user.savedPosts.map((s) => ({ savedAt: s.createdAt, post: serializePost(s.post) })),
...
router.put("/me", requireAuth, async (req, res) => {
  ...
  const user = await prisma.user.update({
    where: { id: req.userId },
    data,
    select: { id: true, name: true, username: true, email: true, bio: true, image: true, xp: true, level: true },
  });
...
router.get("/leaderboard", async (_req, res) => {
  ...
  res.json(
    users.map((u, index) => ({
      ...
      xp: u.xp,
      level: u.level,
      stats: { posts: u._count.posts, ratings: u._count.ratings, reviews: u._count.comments },
