...
router.post("/register", async (req, res) => {
  ...
  if (normalizedUsername) {
    if (!/^[a-zA-Z0-9_.]{3,20}$/.test(normalizedUsername)) {
      return res.status(400).json({ error: "Username 3-20 karakter (huruf, angka, _ atau .)." });
    }
    const clash = await prisma.user.findUnique({ where: { username: normalizedUsername } });
    if (clash) {
...
router.get("/me", requireAuth, async (req, res) => {
  const user = await prisma.user.findUnique({
    ...
    include: {
      ...
      reservations: { include: { place: true }, orderBy: { createdAt: "desc" } },
    },
  });
