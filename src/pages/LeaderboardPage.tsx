...
export default function LeaderboardPage() {
  ...
  return (
    <div className="pt-24 px-6 md:px-12 max-w-4xl mx-auto pb-20">
      ...
      <h1 className="text-4xl md:text-5xl font-black text-foreground mb-2" style={{ fontFamily: "'Fraunces', serif" }}>
        Kontributor Teratas
      </h1>
      <p className="text-muted-foreground text-sm mb-8">
        Dapatkan XP dan lencana dari setiap ulasan, rating, dan postingan yang kamu buat.
      </p>
      ...
      {loading ? (
        ...
        <div className="flex flex-col gap-3">
          {entries.map((e) => (
            <div key={e.id} className="glass-card rounded-2xl p-4 flex items-center gap-4">
              <div className="w-10 shrink-0 text-center text-2xl font-black">
                {MEDALS[e.rank - 1] ?? <span className="text-muted-foreground text-base">#{e.rank}</span>}
              </div>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span className="font-bold text-foreground truncate">{e.name ?? "Pengguna"}</span>
                  <span className="tag-pill">Lv.{e.level}</span>
                </div>
                <div className="text-xs text-muted-foreground mt-0.5">
                  {e.xp.toLocaleString()} XP · {e.stats.posts} postingan · {e.stats.ratings} rating · {e.stats.reviews} ulasan
                </div>
                <div className="mt-2 h-1.5 rounded-full bg-muted overflow-hidden">
                  <div
                    className="h-full rounded-full bg-[#b07d3f]"
                    style={{ width: `${Math.round(levelProgress(e.xp, e.level) * 100)}%` }}
                  />
                </div>
              </div>
