      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid gap-5 lg:grid-cols-[1fr_360px]">
          <main className="space-y-4">
            ...
            <section className="rounded-2xl border border-border bg-card"><div className="flex items-center justify-between border-b border-border px-5 py-4"><h2 className="flex items-center gap-2 font-black text-foreground"><MdStorefront className="text-primary" /> {storeName}</h2><span className="text-xs text-muted-foreground">{items.length} produk</span></div>{items.map((item) => <div key={item.id} className="flex items-center gap-3 border-b border-border px-5 py-4 last:border-0"><img src={menuImageUrl(item.category, item.imageUrl, item.name)} alt={item.name} className="h-16 w-16 rounded-xl object-cover" /><div className="min-w-0 flex-1"><div className="truncate text-sm font-bold text-foreground">{item.name}</div><div className="text-xs text-muted-foreground">Variasi standar · x{item.quantity}</div></div><div className="text-right text-sm font-black text-primary">{formatRupiah(item.price * item.quantity)}<div className="text-xs font-normal text-muted-foreground">{formatRupiah(item.price)} / item</div></div></div>)}<div className="flex items-center gap-2 px-5 py-4"><MdLocalShipping className="text-primary" /><div className="flex-1"><div className="text-sm font-bold text-foreground">J&T Express · Reguler</div><div className="text-xs text-muted-foreground">Estimasi 2–4 hari</div></div><strong className="text-sm text-foreground">{formatRupiah(SHIPPING_FEE)}</strong></div><textarea value={note} onChange={(e) => setNote(e.target.value)} placeholder="Pesan untuk penjual (opsional)" rows={2} className="mx-5 mb-5 w-[calc(100%-2.5rem)] resize-none rounded-xl border border-border bg-background px-4 py-3 text-sm text-foreground outline-none focus:border-primary" /></section>
          </main>
