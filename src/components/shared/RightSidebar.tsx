  ...
  return (
    <aside className="hidden lg:block w-72 shrink-0 sticky top-24 self-start space-y-6">
      <div className="glass-card rounded-2xl p-5">
        <h3 className="text-xs font-bold uppercase tracking-widest text-muted-foreground mb-3">🔥 Kafe Sedang Tren</h3>
        <div className="flex flex-col gap-3">
          {(data?.trendingPlaces ?? []).map((p) => (
            <Link key={p.id} to={`/places/${p.id}`} className="flex items-center gap-3 group">
      ...
      <div className="glass-card rounded-2xl p-5">
        <h3 className="text-xs font-bold uppercase tracking-widest text-muted-foreground mb-3">🏆 Kontributor Teratas</h3>
        <div className="flex flex-col gap-3">
          {(data?.topContributors ?? []).map((u, i) => (
            <div key={u.id} className="flex items-center gap-3">
              <span className="w-5 text-sm font-black text-[#b07d3f]">{i + 1}</span>
      ...
      <div className="glass-card rounded-2xl p-5">
        <h3 className="text-xs font-bold uppercase tracking-widest text-muted-foreground mb-3"># Tag Populer</h3>
      ...
      <div className="glass-card rounded-2xl p-5">
        <h3 className="text-xs font-bold uppercase tracking-widest text-muted-foreground mb-3">💬 Ulasan Terbaru</h3>
        <div className="flex flex-col gap-4">
          {(data?.latestReviews ?? []).map((r) => (
            <div key={r.id} className="border-l-2 border-[rgba(140,95,40,0.3)] pl-3">
              <p className="text-sm text-muted-foreground line-clamp-3 leading-relaxed">{r.body}</p>
              <div className="text-xs mt-1.5">
                <span className="text-foreground font-semibold">{r.user.name}</span>
                <span className="text-muted-foreground"> di </span>
                <Link to={`/places/${r.place.id}`} className="text-[#b07d3f] font-semibold hover:underline">
                  {r.place.name}
                </Link>
